...
function App () {
  ...
  const giveEthValue = async ethValue => {
    const ethReserve = await exchangeContract.methods
      .getEtherReserve()
      .call({ from: accounts[0] })
    const tokenReserve = await exchangeContract.methods
      .getTokenReserve()
      .call({ from: accounts[0] })
    const tokenAmount = await exchangeContract.methods
      .getTokenAmount(
        web3.utils.toWei(ethValue, 'ether'),
        ethReserve.toString(),
  ...
  const approveToken = async tokenValue => {
    ...
    try {
      await TokenContract.methods
        .approve(exchangeContract._address, web3.utils.toWei(tokenValue))
    ...
    return (
      <>
        ...
        <Container maxWidth='md'>
          ...
          <Box mt='1.5rem'>
            <Typography variant='h5' align='center'>
              Please use the Goerli testnet. If you are using another network,
              then please switch over to Goerli and refresh the page!
            </Typography>{' '}
          </Box>
          <Box mt='2rem' component='span'>
            <Typography variant='h6' align='center'>
              If you do not have an etherum wallet, you can install it from
              here:{' '}
            </Typography>
          </Box>
          <Box mt='2rem' component='span'>
        ...
        <Container maxWidth='md'>
          ...
          <Box mt='1.25rem'>
            <Typography variant='h6' align='center' color='textSecondary'>
              Exchange contract address: {exchangeContract._address}
            </Typography>
          </Box>
          <Box mb='1.25rem'>
            <Typography variant='h6' align='center' color='textSecondary'>
              DareToken contract address: {TokenContract._address}
            </Typography>
          </Box>
          ...
          {ethExchange && (
            <Box m='1.5rem'>
              <EthToToken
                ...
                giveEthValue={giveEthValue}
                web3={web3}
                exchangeEthValueToDare={exchangeEthValueToDare}
              />
            </Box>
          )}
          {!ethExchange && (
            <Box m='1.5rem'>
              <TokenToEth
                getEthValue={getEthValue}
                giveTokenValue={giveTokenValue}
                web3={web3}
